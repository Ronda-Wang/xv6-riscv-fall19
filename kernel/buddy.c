...
struct sz_info {
  ...
  char *alloc;//当前层的所有内存块的alloc。每个块只用一个bit，所以alloc指向的数组的大小为（当前层块数/8）字节
  char *split;//当前层的所有内存块的split。每个块只用一个bit
};
typedef struct sz_info Sz_info;//每个size层的信息都被存储在struct sz_info中
...
//******
int
bd_initfree_pair(int k, int bi, int m) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(bit_isset(bd_sizes[k].alloc, bi / 2)){
    int big = (buddy > bi)?buddy:bi;
    int small = (buddy < bi)?buddy:bi;
    free = BLK_SIZE(k);
    if(m)
      lst_push(&bd_sizes[k].free, addr(k, big));
    else
      lst_push(&bd_sizes[k].free, addr(k, small));
  }
  return free;
}
